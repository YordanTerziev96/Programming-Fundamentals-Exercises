...
namespace _05.HandsOfCards
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var playersResult = new Dictionary<string, int>();
            foreach (var entry in playersCards)
            {
                var cards = entry.Value.Split(new char[] { ' ', ',', ':' }, StringSplitOptions.RemoveEmptyEntries)
                    .Distinct()
                    .ToList();
                var finalResult = 0;
                ...
                playersResult[entry.Key] = finalResult;
            }
            foreach (var pair in playersResult)
            {
